...
function createDefaultCityPage(name, slug) {
  ...
  return {
    ...
    heroText: `Fast, friendly, and fully licensed rubbish collection across ${cityName}. From single items to full property clearances, our team handles the heavy lifting, loading, and responsible disposal.`,
    ...
    wasteText: `Need fast waste collection without the delays and hassle of skip hire? Our ${cityName} team offers a quicker, cleaner option for homes and businesses.`,
    ...
    propertyText: `From lofts and basements to garages and full house clearances, we collect bulky waste, mixed rubbish, furniture, and general junk across ${cityName}.`,
...
app.put("/api/admin/city-pages/:id", requireAdminAuth, async (req, res) => {
  const { id } = req.params;
  const siteContent = await readSiteContent();
  ...
  const updatedPage = buildUpdatedCityPage(targetPage, req.body ?? {});
